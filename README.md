...
1. 장바구니 페이지에서 상품의 수량을 늘리거나 줄이면 값이 0원이 되는 문제 발생

    ⇒ **해결방법**: 컴포넌트 return() 안에서 계산식 직접 작성해서 cartItems 상태가 아직 초기화되지 않았고, 아직 렌더링이 되지 않아 빈 배열이었기 때문에 0이 반환되어 나온 것이다. 계산 로직을 컴포넌트 밖의 함수로 분리하고, return 안에서는 함수 호출만 하도록 구조를 바꾸어 해결

---
## **💭** 느낀점
처음 접했던 PHP도 계속 다루다 보니 익숙해졌고, 전보다 더 서버와 데이터베이스(db)간의 흐름을 이해하게 되었다. <br>
프론트엔드 개발자로서 백엔드 구조의 중요성을 다시 한번 느낀 프로젝트였다. <br><br>

짧은 시간 내에 마칠 수 있을까 걱정이 많았지만, 팀원과 상성이 맞고 의견이 비슷해 좋은 시너지로 작업을 효율적으로 완성할 수 있었고,<br>
이번 프로젝트 덕분에 적은 인원으로도 충분히 좋은 결과를 낼 수 있다는 걸 경험할 수 있었다.
